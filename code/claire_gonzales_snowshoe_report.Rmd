...
kable(hare_juv_compare,
      ...
      col.names = c("Sex", # labeling column names with unit
                    ...
                    "Count"),
      caption = "**Table 1** Shows a summary statistics table for body weight distribution with respect to sex of juvenile snowshoe hares captured in Bonanza Creek Experimental Forest between the years 1999 and 2012") %>% 
   kable_styling(bootstrap_options = "striped", full_width = FALSE)
```

The body weight (g) distribution of female and male juvenile snowshoe hares depicted in **Table 1** 

### 3d. Relationship between juvenile weight & hind foot length

```{r, include=FALSE}
juv_foot_weight <- hare_juv_weight %>% #Messed up! (overwrite later)
  group_by(hindft, weight) %>% 
  summarize(hindft = hindft,
            weight = weight)
```

```{r}
ggplot(data = hare_juv_weight, aes(x = hindft, y = weight)) + #Initial visualization of hind foot length and weight of hare
  geom_point() +
...
juvenile_cor_tidy
```
 blah blah `r juvenile_cor_tidy$estimate`
